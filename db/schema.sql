...
-- ---------- Usage Metering (per brand, per billing cycle) ----------
create table if not exists ait_metering (
  id              bigserial primary key,
  brand_id        text not null,
  ...
  messages_sent   int not null default 0,      -- email + whatsapp combined
  api_calls       int not null default 0,
  engine_runs     int not null default 0,      -- total ECM engine invocations
  updated_at      timestamptz not null default now(),
  unique(brand_id, cycle_start)
);
create index if not exists ait_metering_brand on ait_metering(brand_id, cycle_start);

-- ---------- Media Assets (images, thumbnails generated by ECM engines) ----------
create table if not exists ait_media (
  id              bigserial primary key,
  brand_id        text not null,
  post_id         bigint references ait_social_posts(id) on delete set null,
  kind            text not null default 'image',  -- image|video|thumbnail|avatar
  url             text,
  prompt          text,                           -- AI prompt used to generate
  engine          text,                           -- which engine generated it
  meta            jsonb not null default '{}',
  created_at      timestamptz not null default now()
);
create index if not exists ait_media_brand on ait_media(brand_id, kind);

-- ---------- Plan Limits (tier enforcement) ----------
create table if not exists ait_plans (
  id              bigserial primary key,
  name            text not null unique,           -- free|starter|growth|pro
  price_inr       int not null default 0,         -- price in INR per month
  max_leads       int not null default 5,
  max_posts       int not null default 10,        -- posts per month
  max_sequences   int not null default 1,         -- outreach sequences
  max_social_acc  int not null default 1,         -- social accounts
  max_messages    int not null default 50,        -- emails + whatsapp per month
  features        jsonb not null default '[]',    -- feature flags
  created_at      timestamptz not null default now()
);
-- seed default plans
insert into ait_plans (name, price_inr, max_leads, max_posts, max_sequences, max_social_acc, max_messages, features)
values
  ('free',    0,    5,   10, 1, 1,  50,  '["basic_content","basic_outreach"]'),
  ('starter', 1500, 25,  40, 1, 1, 200,  '["content","outreach","analytics"]'),
  ('growth',  3000, 100, 999, 5, 3, 999,  '["content","outreach","analytics","priority_ai","social_publish"]'),
  ('pro',     6000, -1,  -1, -1, -1, -1,  '["content","outreach","analytics","priority_ai","social_publish","custom_branding","api_access"]')
